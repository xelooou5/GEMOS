#!/usr/bin/env python3
"""
💎 GEM OS - Main Application Entry Point
...
class GEM:
    ...
    def initialize(self):
        ...
        print("💎 Initializing GEM OS...")

        try:
            self.audio = AudioSystem(self.config_manager)
            self.audio.initialize()
            print("🎙️ Audio system initialized.")
        except Exception as e:
            self.failed_modules.append("audio")
            print(f"⚠️ Failed to init AudioSystem: {e}")

        try:
            self.stt = STTModule(self.config_manager)
            print("🗣️ STT module initialized.")
        except Exception as e:
            self.failed_modules.append("stt")
            print(f"⚠️ Failed to init STTModule: {e}")
        ...
        try:
            ...
            print(f"⚠️ Failed to init TTSModule: {e}")
        ...
        try:
            ...
            print(f"⚠️ Failed to init LLMHandler: {e}")
        ...
        try:
            ...
            print(f"⚠️ Failed to init CommandExecutor: {e}")

        if self.failed_modules:
            print(f"⚠️ GEM started in degraded mode. Failed: {', '.join(self.failed_modules)}")
    ...
    async def run(self, dry_run: bool = False):
        """
        Main asynchronous loop for wake-word detection, STT → LLM → TTS → Command execution.
        """
        self.running = True
        self.greet_user()

        if dry_run:
            print("🔧 Dry run mode: initialized all systems, skipping main loop.")
            return
...
def parse_args():
    parser = argparse.ArgumentParser(description="💎 GEM OS - Voice Assistant")
