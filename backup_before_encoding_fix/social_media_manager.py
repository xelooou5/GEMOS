...
# -*- coding: utf-8 -*-
"""
💎 GEM OS - Social Media Manager (core/social_media_manager.py)
...
# --- Dataclass para Postagem de Mídia Social ---
@dataclass
...
class SocialMediaManager(BasePlugin):
    ...
    def __init__(self, gem_instance: GEMVoiceAssistant, logger: Optional[logging.Logger] = None):
        ...
        if self._twitter_api_key == "MOCK_TWITTER_KEY" or \
           ...
           self._instagram_api_token == "MOCK_INSTAGRAM_TOKEN":
            self.logger.warning("Credenciais de mídia social estão usando valores de MOCK. Funcionalidade real será limitada.")
    ...
    async def _post_update_to_platform(self, platform: str, message: str,
        ...
        if platform.lower() == "twitter":
            ...
            return {"success": False, "output": "", "error": f"Plataforma '{platform}' não suportada."}
        ...
        try:
            ...
            error_msg = f"Erro de rede/requisição ao postar para {platform}: {e}"
            self.logger.error(error_msg, exc_info=True)
    ...
    async def _get_feed_from_platform(self, platform: str, limit: int = 3) -> List[SocialMediaPost]:
        ...
        if platform.lower() == "twitter":
            ...
            self.logger.warning(f"Plataforma '{platform}' não suportada para obter feed.")
            return []

        if "MOCK" in headers.get("Authorization", ""):
            self.logger.warning(f"Usando credenciais mock para {platform}. Obtenção de feed apenas simulada.")
        ...
        self.logger.info(f"Simulando obtenção de feed para {platform}...")
        try:
            ...
            error_msg = f"Erro de rede/requisição ao obter feed de {platform}: {e}"
            self.logger.error(error_msg, exc_info=True)
    ...
    async def _post_social_update_command(self, platform: str, message: str, media_url: Optional[str] = None) -> Dict[str, Any]:
        """
        Posta uma atualização de texto ou com mídia para uma plataforma de mídia social.
        Plataformas suportadas: "twitter", "facebook", "instagram".
        """
        if not message:
            return {"success": False, "output": "", "error": "A mensagem não pode estar vazia."}
        
        platforms_supported = ["twitter", "facebook", "instagram"]
        if platform.lower() not in platforms_supported:
            await self._speak_response(f"Plataforma de mídia social '{platform}' não suportada. Tente: {', '.join(platforms_supported)}.")
            return {"success": False, "output": "", "error": f"Plataforma '{platform}' não suportada."}

        result = await self._post_update_to_platform(platform, message, media_url)
        if result["success"]:
            output_message = f"Postagem para {platform} concluída: {result['output']}"
            ...
            await self._speak_response(f"Não foi possível postar em {platform}. Verifique os logs.")
            return result # Pass through error

    async def _get_social_feed_command(self, platform: str, limit: int = 3) -> Dict[str, Any]:
        """
        Obtém e apresenta o feed de notícias ou postagens de uma plataforma de mídia social.
        Plataformas suportadas: "twitter", "facebook", "instagram".
        """
        platforms_supported = ["twitter", "facebook", "instagram"]
        if platform.lower() not in platforms_supported:
            await self._speak_response(f"Plataforma de mídia social '{platform}' não suportada para feed. Tente: {', '.join(platforms_supported)}.")
            return {"success": False, "output": "", "error": f"Plataforma '{platform}' não suportada."}
        ...
        if not posts:
            message = f"Não foram encontrados posts no feed de {platform}."
            await self._speak_response(message)
            return {"success": True, "output": message, "error": None}

        output_lines = [f"Feed de {platform} (últimos {len(posts)} posts):"]
        for i, post in enumerate(posts):
            output_lines.append(f"{i+1}. De: {post.author} (em {post.timestamp.strftime('%Y-%m-%d %H:%M')})")
            output_lines.append(f"   Conteúdo: {post.content}")
            if post.url:
                output_lines.append(f"   URL: {post.url}")
            output_lines.append("-" * 20)
        
        message = "\n".join(output_lines)
        await self._speak_response(f"O feed de {platform} foi obtido. Verifique o ecrã para os posts mais recentes.")
        return {"success": True, "output": message, "error": None}
        ...
        return [
            {
                ...
                "function": {
                    ...
                    "description": "Posta uma atualização de texto ou com mídia (opcional) em uma plataforma de mídia social configurada (Twitter, Facebook, Instagram). Requer a plataforma e a mensagem.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "platform": {
                                "type": "string",
                                "description": "A plataforma de mídia social para postar (ex: 'twitter', 'facebook', 'instagram').",
                                "enum": ["twitter", "facebook", "instagram"]
                            },
                            "message": {
                                "type": "string",
                                "description": "O conteúdo da mensagem a ser postada.",
                            },
                            "media_url": {
                                "type": "string",
                                "description": "Um URL para anexar mídia (imagem/vídeo) à postagem. Opcional.",
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Obtém os posts mais recentes do feed de uma plataforma de mídia social configurada (Twitter, Facebook, Instagram).",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "platform": {
                                "type": "string",
                                "description": "A plataforma de mídia social da qual obter o feed (ex: 'twitter', 'facebook', 'instagram').",
                                "enum": ["twitter", "facebook", "instagram"]
                            },
                            "limit": {
                                "type": "integer",
                                "description": "O número máximo de posts a retornar. Padrão para 3, máximo de 10.",
                                "default": 3,
                                "minimum": 1,
                                "maximum": 10
                            }
...
if __name__ == "__main__":
    ...
    async def run_social_media_manager_tests():
        ...
        # --- Teste 1: Postar Atualização no Twitter (Mock) ---
        print("\n--- Teste 1: Postar Atualização no Twitter (Mock) ---")
        result_post_twitter = await dummy_gem.command_executor.execute(
            "post_social_update", platform="twitter", message="Olá, GEM OS está online!"
        )
        ...
        assert "Feed de facebook (últimos 1 posts):" in result_get_feed_facebook["output"]
        assert "De: MockUser1" in result_get_feed_facebook["output"]

        # --- Teste 3: Tentar Postar em Plataforma Não Suportada ---
        print("\n--- Teste 3: Tentar Postar em Plataforma Não Suportada ---")
        result_post_invalid = await dummy_gem.command_executor.execute(
            "post_social_update", platform="linkedin", message="Isto não deve funcionar."
        )
        print(result_post_invalid["output"])
        assert result_post_invalid["success"] is False
        assert "Plataforma 'linkedin' não suportada." in result_post_invalid["error"]
        ...
        assert "A mensagem não pode estar vazia." in result_post_empty["error"]

        # --- Teste 5: Postar com Mídia (Mock) ---
        print("\n--- Teste 5: Postar com Mídia (Mock) ---")
        result_post_media = await dummy_gem.command_executor.execute(
            "post_social_update", platform="instagram", message="Nova foto do pôr do sol!", media_url="https://example.com/sunset.jpg"
        )
        print(result_post_media["output"])
        assert result_post_media["success"] is True
        assert "Postagem simulada para instagram: 'Nova foto do pôr do sol!'..." in dummy_gem.notification_manager._history[-1]["message"]


        print("\n--- Testes do SocialMediaManager concluídos com sucesso. ---")
