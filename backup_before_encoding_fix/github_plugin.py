...
# -*- coding: utf-8 -*-
"""
🐙 GEM OS - GitHub Plugin (plugins/github_plugin.py)
...
def list_repos() -> str:
    ...
    if not cfg.get("token") or not cfg.get("user"):
        return "⚠️ GitHub config missing (data/github_config.json)."
...
def list_issues(repo: str) -> str:
    ...
    cfg = _load_config()
    if not cfg.get("token") or not cfg.get("user"):
        return "⚠️ GitHub config missing."
    ...
    try:
        ...
        if not issues:
            return f"📭 Nenhuma issue em {repo}."
        return f"🐞 Issues em {repo}:\n" + "\n".join(issues)
    except Exception as e:
...
def create_issue(repo: str, title: str, body: str = "") -> str:
    ...
    try:
        ...
        r.raise_for_status()
        issue = r.json()
        return f"✅ Issue criada em {repo}: #{issue['number']} - {issue['title']}"
    except Exception as e:
