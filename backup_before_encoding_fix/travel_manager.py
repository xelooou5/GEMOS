...
# -*- coding: utf-8 -*-
"""
💎 GEM OS - Travel Manager (core/travel_manager.py)
...
# --- Dataclass para Itinerário ---
@dataclass
class ItineraryItem:
    id: str
    name: str # Nome do item do itinerário (ex: "Voo para Paris", "Hotel em Roma")
...
class TravelManager(BasePlugin):
    ...
    def __init__(self, gem_instance: GEMVoiceAssistant, logger: Optional[logging.Logger] = None):
        ...
        if self._mock_api_key == "MOCK_TRAVEL_KEY":
            self.logger.warning("Credenciais de viagem estão usando valores de MOCK. Funcionalidade real será limitada.")
        ...
        try:
            ...
            for item_dict in itineraries_data:
                try:
                    ...
                    self.logger.error(f"Erro ao carregar item do itinerário: {e} - Dados: {item_dict}", exc_info=True)
            self.logger.info(f"Carregados {len(self._itineraries)} itens do itinerário do armazenamento.")
        except Exception as e:
            self.logger.error(f"Falha ao carregar itinerários do armazenamento: {e}", exc_info=True)
        finally:
            self._itineraries_loaded.set() # Sinaliza que os itinerários foram carregados
    ...
    async def _save_itineraries_to_storage(self) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Salvos {len(self._itineraries)} itens do itinerário para o armazenamento.")
        except Exception as e:
            self.logger.error(f"Falha ao salvar itinerários no armazenamento: {e}", exc_info=True)
    ...
    async def _search_flights_api(self, origin: str, destination: str, departure_date: str,
        ...
        if "MOCK_TRAVEL_KEY" in self._mock_api_key:
            ...
            await self.notification_manager.add_notification(
                f"Busca simulada de voos para {origin}-{destination} concluída.", level=NOTIFICATION_INFO
            )
        ...
        try:
            ...
            await self.notification_manager.add_notification(
                f"Busca de voos para {origin}-{destination} concluída. {len(flights)} resultados.", level=NOTIFICATION_SUCCESS, vocalize=False
            )
            ...
            error_msg = f"Erro de rede/requisição ao buscar voos: {e}"
            self.logger.error(error_msg, exc_info=True)
    ...
    async def _search_hotels_api(self, city: str, check_in_date: str, check_out_date: str,
        ...
        if "MOCK_TRAVEL_KEY" in self._mock_api_key:
            self.logger.warning(f"Usando credenciais mock para viagens. Busca de hotéis apenas simulada.")
            ...
            await self.notification_manager.add_notification(
                f"Busca simulada de hotéis para {city} concluída.", level=NOTIFICATION_INFO
            )
            return mock_hotels[:limit]

        self.logger.info(f"Simulando busca de hotéis para {city}...")
        try:
            response = await self._http_client.get(search_url, params=params, headers=headers, timeout=15.0)
            response.raise_for_status()
            data = response.json()
            ...
            await self.notification_manager.add_notification(
                f"Busca de hotéis para {city} concluída. {len(hotels)} resultados.", level=NOTIFICATION_SUCCESS, vocalize=False
            )
            await self.event_manager.publish("TRAVEL_HOTEL_SEARCHED", {"city": city, "count": len(hotels)})
            return hotels

        except httpx.HTTPStatusError as e:
            error_msg = f"Erro HTTP ao buscar hotéis: {e.response.status_code} - {e.response.text}"
            self.logger.error(error_msg, exc_info=True)
            await self.notification_manager.add_notification(f"Falha na busca de hotéis: {e.response.status_code}", level=NOTIFICATION_ERROR)
            await self.tts_module.speak(f"Desculpe, houve um erro ao buscar hotéis.")
            return []
        except httpx.RequestError as e:
            error_msg = f"Erro de rede/requisição ao buscar hotéis: {e}"
            self.logger.error(error_msg, exc_info=True)
            await self.notification_manager.add_notification(f"Erro de rede na busca de hotéis.", level=NOTIFICATION_ERROR)
            await self.tts_module.speak(f"Desculpe, houve um problema de rede ao buscar hotéis.")
            return []
        except Exception as e:
            error_msg = f"Erro inesperado ao buscar hotéis: {e}"
            self.logger.error(error_msg, exc_info=True)
            await self.notification_manager.add_notification(f"Erro inesperado na busca de hotéis.", level=NOTIFICATION_ERROR)
            await self.tts_module.speak(f"Desculpe, ocorreu um erro inesperado ao buscar hotéis.")
            return []
    ...
    async def _search_flights_command(self, origin: str, destination: str, departure_date: str,
        ...
        try:
            ...
            await self._speak_response("Formato de data inválido. Use 'AAAA-MM-DD'.")
            return {"success": False, "output": "Formato de data inválido.", "error": "Invalid date format"}
        
        if not (1 <= passengers <= 9):
            return {"success": False, "output": "", "error": "Número de passageiros deve ser entre 1 e 9."}
        if not (1 <= limit <= 10):
        ...
        if not flights:
            message = f"Não foram encontrados voos para {origin} para {destination} na data especificada."
        ...
        for i, flight in enumerate(flights):
            ...
            output_lines.append(f"   Preço: {flight.price:.2f} {flight.currency}")
            if flight.booking_url:
                output_lines.append(f"   Reservar: {flight.booking_url}")
            output_lines.append("-" * 20)
        
        message = "\n".join(output_lines)
        await self._speak_response(f"Voos encontrados. Verifique o ecrã para os detalhes.")
        return {"success": True, "output": message, "error": None}
    ...
    async def _search_hotels_command(self, city: str, check_in_date: str, check_out_date: str,
        ...
        try:
            ...
            await self._speak_response(f"Formato de data inválido ou datas incorretas: {e}. Use 'AAAA-MM-DD'.")
            return {"success": False, "output": "Formato de data inválido.", "error": str(e)}

        if not (1 <= guests <= 9):
            return {"success": False, "output": "", "error": "Número de hóspedes deve ser entre 1 e 9."}
        if stars and not (1 <= stars <= 5):
            return {"success": False, "output": "", "error": "O número de estrelas deve ser entre 1 e 5."}
        if not (1 <= limit <= 10):
            return {"success": False, "output": "", "error": "O limite de resultados deve ser entre 1 e 10."}
        ...
        if not hotels:
            message = f"Não foram encontrados hotéis em {city} para as datas especificadas."
            await self._speak_response(message)
            return {"success": True, "output": message, "error": None}
        
        output_lines = [f"Hotéis encontrados em {city} (Check-in: {check_in_date}, Check-out: {check_out_date}):"]
        for i, hotel in enumerate(hotels):
            stars_str = "⭐" * hotel.stars if hotel.stars else ""
            output_lines.append(f"{i+1}. {hotel.name} {stars_str} - Preço por noite: {hotel.price_per_night:.2f} {hotel.currency}")
            if hotel.booking_url:
                output_lines.append(f"   Reservar: {hotel.booking_url}")
            output_lines.append("-" * 20)
        
        message = "\n".join(output_lines)
        await self._speak_response(f"Hotéis encontrados. Verifique o ecrã para os detalhes.")
        return {"success": True, "output": message, "error": None}
    ...
    async def _add_itinerary_item_command(self, name: str, item_type: str, start_time: str,
        ...
        """
        Adiciona um item ao itinerário de viagem (ex: um voo, uma reserva de hotel, uma atividade).
        `start_time` e `end_time` devem ser no formato ISO 8601 (YYYY-MM-DD HH:MM).
        `details_json` deve ser uma string JSON com detalhes específicos (ex: dados de voo ou hotel).
        """
        ...
        try:
            ...
            await self._speak_response("Formato de data/hora inválido. Use 'AAAA-MM-DD HH:MM'.")
        ...
        if details_json:
            try:
                ...
                await self._speak_response("Formato JSON inválido para detalhes. Por favor, corrija.")
                return {"success": False, "output": "Formato de detalhes JSON inválido.", "error": "Invalid JSON for details"}
        ...
        message = f"Item '{name}' (tipo: {item_type}) adicionado ao seu itinerário."
        await self._speak_response(message)
    ...
    async def _list_itinerary_command(self, start_date: Optional[str] = None, end_date: Optional[str] = None) -> Dict[str, Any]:
        """
        Lista itens do itinerário, opcionalmente filtrando por um intervalo de datas.
        ...
        filtered_items.sort(key=lambda item: item.start_time) # Ordenar por tempo de início

        if not filtered_items:
            message = "Nenhum item encontrado no itinerário."
            if start_date or end_date:
                message = f"Nenhum item encontrado no itinerário para o período de {start_date or 'início'} a {end_date or 'fim'}."
            await self._speak_response(message)
            return {"success": True, "output": message, "error": None}

        output_lines = ["Seu Itinerário de Viagem:"]
        for i, item in enumerate(filtered_items):
            ...
            if item.details:
                for key, value in item.details.items():
                    if key not in ["id", "booking_url"] and isinstance(value, (str, int, float)): # Evitar IDs e URLs grandes, focar em informação relevante
        ...
        message = "\n".join(output_lines)
        await self._speak_response(f"Seu itinerário foi listado. Verifique o ecrã para os detalhes.")
        await self.notification_manager.add_notification("Itinerário exibido.", level=NOTIFICATION_INFO)
        return {"success": True, "output": message, "error": None}

    async def _remove_itinerary_item_command(self, item_id_prefix: str) -> Dict[str, Any]:
        """
        Remove um item do itinerário de viagem.
        ...
        if len(matching_items) == 1:
            ...
            message = f"Múltiplos itens correspondem ao ID '{item_id_prefix}'. Por favor, seja mais específico."
            await self._speak_response(message)
            return {"success": False, "output": message, "error": "Multiple matches"}
        else:
            message = f"Nenhum item encontrado com o ID '{item_id_prefix}' no seu itinerário."
            await self._speak_response(message)
        ...
        if item_to_remove:
            ...
            await self._save_itineraries_to_storage()
            message = f"Item '{item_to_remove.name}' (ID: {item_to_remove.id[:8]}...) removido do seu itinerário."
            await self._speak_response(message)
            await self.notification_manager.add_notification(message, level=NOTIFICATION_SUCCESS)
        ...
        return {"success": False, "output": "Erro desconhecido ao remover item do itinerário.", "error": "Unknown error"}
        ...
        return [
            {
                ...
                "function": {
                    "name": "search_flights",
                    "description": "Pesquisa voos de uma origem para um destino em uma data de partida específica. Opcionalmente, pode incluir uma data de retorno e o número de passageiros. As datas devem ser no formato YYYY-MM-DD.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "origin": {
                                "type": "string",
                                "description": "O código IATA do aeroporto ou nome da cidade de partida (ex: 'LIS', 'Lisboa').",
                            },
                            "destination": {
                                "type": "string",
                                "description": "O código IATA do aeroporto ou nome da cidade de chegada (ex: 'CDG', 'Paris').",
                            },
                            "departure_date": {
                                "type": "string",
                            ...
                            "passengers": {
                                "type": "integer",
                                "description": "O número de passageiros. Padrão para 1.",
                                "default": 1,
                                "minimum": 1,
                                "maximum": 9
                            },
                            "limit": {
                                "type": "integer",
                                "description": "O número máximo de voos a retornar. Padrão para 3, máximo de 10.",
                                "default": 3,
                                "minimum": 1,
                                "maximum": 10
                            }
            ...
            },
            {
                ...
                "function": {
                    "name": "search_hotels",
                    "description": "Pesquisa hotéis em uma cidade para um período de check-in e check-out. Opcionalmente, pode especificar o número de hóspedes e a classificação por estrelas. As datas devem ser no formato YYYY-MM-DD.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "city": {
                                "type": "string",
                                "description": "O nome da cidade para pesquisar hotéis (ex: 'Paris').",
                            },
                            "check_in_date": {
                                "type": "string",
                            ...
                            "guests": {
                                "type": "integer",
                                "description": "O número de hóspedes. Padrão para 1.",
                                "default": 1,
                                "minimum": 1,
                                "maximum": 9
                            },
                            "stars": {
                                "type": "integer",
                                "description": "A classificação mínima por estrelas do hotel (1 a 5). Opcional.",
                                "minimum": 1,
                                "maximum": 5
                            },
                            "limit": {
                                "type": "integer",
                                "description": "O número máximo de hotéis a retornar. Padrão para 3, máximo de 10.",
                                "default": 3,
                                "minimum": 1,
                                "maximum": 10
                            }
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Adiciona um item ao seu itinerário de viagem (ex: um voo, uma reserva de hotel, uma atividade). Requer um nome, tipo de item e hora de início. Opcionalmente, pode incluir hora de término, localização e detalhes adicionais em formato JSON.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "name": {
                                "type": "string",
                                "description": "Um nome descritivo para o item do itinerário (ex: 'Voo para Paris', 'Jantar na Torre Eiffel').",
                            },
                            "item_type": {
                                "type": "string",
                                "description": "O tipo do item do itinerário (ex: 'flight', 'hotel', 'activity', 'general').",
                                "enum": ["flight", "hotel", "activity", "general"]
                            },
                            "start_time": {
                                "type": "string",
                                "description": "A data e hora de início do item no formato ISO 8601 (YYYY-MM-DD HH:MM).",
                            },
                            "end_time": {
                                "type": "string",
                                "description": "A data e hora de término do item no formato ISO 8601 (YYYY-MM-DD HH:MM). Opcional.",
                            },
                            "location": {
                                "type": "string",
                                "description": "A localização associada ao item (ex: 'Aeroporto de Lisboa', 'Hotel Mock Eiffel'). Opcional.",
                            },
                            "details_json": {
                                "type": "string",
                                "description": "Uma string JSON com detalhes adicionais específicos do item (ex: {'flight_number': 'AM100', 'airline': 'AirMock'}). Opcional.",
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    "name": "list_itinerary",
                    "description": "Lista todos os itens no itinerário de viagem do usuário, opcionalmente filtrando por um intervalo de datas. Ordena os itens por ordem cronológica.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "start_date": {
                                "type": "string",
                                "description": "A data de início do intervalo para filtrar o itinerário no formato YYYY-MM-DD. Opcional.",
                            },
                            "end_date": {
                                "type": "string",
                                "description": "A data de término do intervalo para filtrar o itinerário no formato YYYY-MM-DD. Opcional.",
                            }
                        },
                        "required": [],
                    },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Remove um item específico do itinerário de viagem. Requer o ID completo ou um prefixo único do item.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "item_id_prefix": {
                                "type": "string",
                                "description": "O ID completo ou um prefixo único do item do itinerário a ser removido.",
                            },
                        },
...
if __name__ == "__main__":
    ...
    async def run_travel_manager_tests():
        ...
        assert "Hotéis encontrados em Paris" in result_search_hotels["output"]
        assert "Hotel Mock Eiffel" in result_search_hotels["output"]
        
        # --- Teste 3: Adicionar Item ao Itinerário (Voo) ---
        print("\n--- Teste 3: Adicionar Item ao Itinerário (Voo) ---")
        flight_details = {
            "airline": "AirMock",
            "flight_number": "AM100",
        ...
        assert "Item 'Voo para Paris' (tipo: flight) adicionado ao seu itinerário." in result_add_flight_item["output"]

        # --- Teste 4: Adicionar Item ao Itinerário (Hotel) ---
        print("\n--- Teste 4: Adicionar Item ao Itinerário (Hotel) ---")
        hotel_details = {
        ...
        assert "Item 'Estadia em Hotel Paris' (tipo: hotel) adicionado ao seu itinerário." in result_add_hotel_item["output"]


        # --- Teste 5: Listar Itinerário ---
        print("\n--- Teste 5: Listar Itinerário ---")
        ...
        assert "Seu Itinerário de Viagem:" in result_list_itinerary["output"]

        # --- Teste 6: Remover Item do Itinerário ---
        print("\n--- Teste 6: Remover Item do Itinerário ---")
        ...
        assert "Item 'Voo para Paris' (ID:" in result_remove_item["output"] and "removido do seu itinerário." in result_remove_item["output"]
        ...
        assert "Não foram encontrados voos para XYZ para ABC na data especificada." in result_no_flights["output"]

        print("\n--- Testes do TravelManager concluídos com sucesso. ---")
