...
# -*- coding: utf-8 -*-
"""
💎 GEM OS - Calendar Manager (core/calendar_manager.py)
...
# --- Dataclass para Evento de Calendário ---
@dataclass
class CalendarEvent:
    id: str
    title: str
...
class CalendarManager(BasePlugin):
    ...
    async def initialize(self) -> None:
        ...
        if not self._check_task:
            ...
            self.logger.info("Verificação periódica de eventos de calendário iniciada.")
    ...
    async def _on_gem_shutdown(self, event_data: Dict[str, Any]) -> None:
        ...
        self.logger.info("Recebido GEM_SHUTDOWN. A parar as verificações de eventos de calendário.")
        self.shutdown()
    ...
    async def _load_events_from_storage(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Carregados {len(self._events)} eventos do calendário do armazenamento.")
        except Exception as e:
            self.logger.error(f"Falha ao carregar eventos do calendário do armazenamento: {e}", exc_info=True)
        finally:
    ...
    async def _periodic_event_check(self) -> None:
        ...
        self.logger.debug("Loop de verificação periódica de eventos de calendário iniciado.")
        ...
        while True:
            try:
                ...
                self.logger.info("Tarefa de verificação periódica de eventos de calendário cancelada.")
                break
            except Exception as e:
                self.logger.error(f"Erro no loop de verificação de eventos de calendário: {e}", exc_info=True)
        ...
        for event_id, event in list(self._events.items()):
            ...
            if event.start_time > now and (event.start_time - now) < timedelta(minutes=5) and \
               not any("start_alert" in r for r in event.reminders): # Prevent duplicate alerts
                self.logger.info(f"Evento '{event.title}' a começar em breve: {event.start_time.strftime('%H:%M')}.")
                message = f"Lembrete: O evento '{event.title}' começa em breve, às {event.start_time.strftime('%H:%M')}."
            ...
            for reminder_time in event.reminders:
                if isinstance(reminder_time, datetime) and reminder_time <= now and \
                   (now - reminder_time) < timedelta(minutes=1): # Trigger if within 1 minute of now
                    ...
                    message = f"Lembrete: '{event.title}' está a acontecer agora!"
    ...
    async def _create_calendar_event_command(self, title: str, start_time: str, end_time: str,
        ...
        reminders é uma lista de strings de data/hora no mesmo formato.
        """
        ...
        if parsed_start_time >= parsed_end_time:
            await self._speak_response("A hora de início deve ser anterior à hora de término.")
        ...
        if reminders:
            for r_str in reminders:
                try:
                    ...
                    await self._speak_response(f"Formato de lembrete inválido: '{r_str}'. Ignorando este lembrete.")
                    self.logger.warning(f"Formato de lembrete inválido: '{r_str}'. Ignorando.")
    ...
    async def _list_calendar_events_command(self, period: str = "today") -> Dict[str, Any]:
        """
        Lista eventos do calendário para um determinado período.
        Períodos suportados: "today", "tomorrow", "this_week", "next_7_days", "all".
        """
        await self._events_loaded.wait() # Ensure events are loaded
        ...
        if not filtered_events:
            message = f"Não há eventos para o período '{period}'."
            await self._speak_response(message)
            return {"success": True, "output": message, "error": None}

        output_lines = [f"Eventos para {period}:"]
        for i, event in enumerate(filtered_events):
            output_lines.append(f"{i+1}. {event.title} (Início: {event.start_time.strftime('%Y-%m-%d %H:%M')}, Fim: {event.end_time.strftime('%H:%M')})")
            if event.location:
                output_lines.append(f"   Local: {event.location}")
            if event.description:
                output_lines.append(f"   Descrição: {event.description}")
            output_lines.append(f"   (ID: {event.id[:8]}...)")
            output_lines.append("-" * 20)
        
        message = "\n".join(output_lines)
        await self._speak_response(f"Os eventos para {period} foram listados. Verifique o ecrã para os detalhes.")
        return {"success": True, "output": message, "error": None}
    ...
    async def _update_calendar_event_command(self, event_id_prefix: str, new_title: Optional[str] = None,
        ...
        if len(matching_events) == 1:
            ...
            message = f"Múltiplos eventos correspondem ao ID '{event_id_prefix}'. Por favor, seja mais específico."
            await self._speak_response(message)
        ...
        if event_to_update:
            ...
            try:
                ...
                await self._speak_response("Formato de data/hora inválido para atualização. Por favor, use 'AAAA-MM-DD HH:MM'.")
                return {"success": False, "output": "Formato de data/hora inválido.", "error": "Invalid date/time format"}

            if event_to_update.start_time >= event_to_update.end_time:
                await self._speak_response("A hora de início atualizada deve ser anterior à hora de término.")
    ...
    async def _delete_calendar_event_command(self, event_id_prefix: str) -> Dict[str, Any]:
        """
        Remove um evento de calendário usando um prefixo do ID.
        """
        await self._events_loaded.wait() # Ensure events are loaded
        ...
        return [
            {
                ...
                "function": {
                    ...
                    "description": "Cria um novo evento de calendário com um título, hora de início e hora de término. Opcionalmente pode incluir uma descrição, localização e lembretes.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "title": {
                            ...
                            "start_time": {
                                "type": "string",
                                "description": "A data e hora de início do evento no formato ISO 8601 (YYYY-MM-DD HH:MM).",
                            },
                            "end_time": {
                                "type": "string",
                                "description": "A data e hora de término do evento no formato ISO 8601 (YYYY-MM-DD HH:MM).",
                            },
                            "description": {
                                "type": "string",
                                "description": "Uma descrição detalhada do evento. Opcional.",
                            },
                            "location": {
                                "type": "string",
                                "description": "A localização do evento. Opcional.",
                            },
                            "reminders": {
                                "type": "array",
                                "items": {"type": "string"},
                                "description": "Uma lista de datas/horas no formato ISO 8601 (YYYY-MM-DD HH:MM) para lembretes específicos. Opcional.",
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Lista os eventos de calendário para um período específico. Pode listar eventos para 'today' (hoje), 'tomorrow' (amanhã), 'this_week' (esta semana), 'next_7_days' (próximos 7 dias) ou 'all' (todos).",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "period": {
                                "type": "string",
                                "description": "O período para o qual listar os eventos. Padrão para 'today'.",
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Atualiza os detalhes de um evento de calendário existente. Requer o ID completo ou um prefixo único do evento. Pode atualizar o título, horários, descrição, localização e lembretes.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "event_id_prefix": {
                                "type": "string",
                                "description": "O ID completo ou um prefixo único do evento a ser atualizado.",
                            },
                            "new_title": {
                                "type": "string",
                                "description": "Um novo título para o evento. Opcional.",
                            },
                            "new_start_time": {
                                "type": "string",
                                "description": "Uma nova data e hora de início no formato ISO 8601 (YYYY-MM-DD HH:MM). Opcional.",
                            },
                            "new_end_time": {
                                "type": "string",
                                "description": "Uma nova data e hora de término no formato ISO 8601 (YYYY-MM-DD HH:MM). Opcional.",
                            },
                            "new_description": {
                                "type": "string",
                                "description": "Uma nova descrição para o evento. Opcional.",
                            },
                            "new_location": {
                                "type": "string",
                                "description": "Uma nova localização para o evento. Opcional.",
                            },
                            "new_reminders": {
                                "type": "array",
                                "items": {"type": "string"},
                                "description": "Uma nova lista de datas/horas para lembretes no formato ISO 8601 (YYYY-MM-DD HH:MM). Opcional. Se fornecido, substituirá os lembretes existentes.",
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Remove um evento de calendário existente. Requer o ID completo ou um prefixo único do evento.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "event_id_prefix": {
                                "type": "string",
                                "description": "O ID completo ou um prefixo único do evento a ser removido.",
                            }
                        },
                        "required": ["event_id_prefix"],
                    },
    ...
    def shutdown(self) -> None:
        ...
        self.logger.info("CalendarManager a ser desligado. A parar a tarefa de verificação de eventos.")
        if self._check_task:
...
if __name__ == "__main__":
    ...
    async def run_calendar_manager_tests():
        ...
        assert "Evento 'Reunião de Equipa' criado" in result_create1["output"]
        ...
        assert "Reunião de Equipa" in result_list_today["output"]
        assert "Nenhum evento para o período 'today'." not in result_list_today["output"]

        print("\n--- Teste 3: Listar Eventos de Amanhã ---")
        ...
        assert "Consulta Médica" in result_list_tomorrow["output"]

        print("\n--- Teste 4: Verificar lembrete de evento (aguardando 5 segundos) ---")
        print("Aguardando 5 segundos para o lembrete da 'Reunião de Equipa'...")
        await asyncio.sleep(6) # Aguarda o lembrete (configurado para 5s antes do início do evento)

        notification_history = dummy_gem.notification_manager.get_notification_history(limit=10)
        assert any("Lembrete: 'Reunião de Equipa' está a acontecer agora!" in n["message"] for n in notification_history)
        print("✅ Lembrete 'Reunião de Equipa' verificado no histórico de notificações.")

        print("\n--- Teste 5: Atualizar Evento ---")
        event_id_to_update = next(e.id for e in calendar_manager._events.values() if e.title == "Reunião de Equipa")
        ...
        assert "Evento 'Reunião de Projeto' (ID:" in result_update["output"] and "atualizado com sucesso." in result_update["output"]
        
        # Verify update
        updated_event = calendar_manager._events[event_id_to_update]
        assert updated_event.title == "Reunião de Projeto"
        ...
        assert "Reunião de Projeto" in result_list_all["output"]
        assert "Consulta Médica" in result_list_all["output"]

        print("\n--- Teste 7: Remover Evento ---")
        event_id_to_delete = next(e.id for e in calendar_manager._events.values() if e.title == "Consulta Médica")
        ...
        assert "Evento 'Consulta Médica' (ID:" in result_delete["output"] and "removido." in result_delete["output"]
        
        # Verify deletion
        assert event_id_to_delete not in calendar_manager._events

        print("\n--- Testes do CalendarManager concluídos com sucesso. ---")
