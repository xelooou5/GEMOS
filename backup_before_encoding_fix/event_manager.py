...
# -*- coding: utf-8 -*-
"""
💎 GEM OS - Event Manager (core/event_manager.py)
...
class EventManager:
    ...
    def unsubscribe(self, event_type: str, listener: EventListener) -> None:
        ...
        if listener in self._listeners[event_type]:
            ...
            self.logger.warning(f"Tentativa de desubcrever um listener não registado para '{event_type}': {listener.__name__}")
...
if __name__ == "__main__":
    ...
    async def listener_c_unsubscribes(event_data: Dict[str, Any]) -> None:
        ...
        # Simula um listener que se desubcreve a si próprio
        global event_manager # Aceder à instância global para desubcrever
        if event_manager:
    ...
    async def run_event_manager_tests():
        ...
        print("\n--- Teste 1: Subcrever e Publicar eventos básicos ---")
        event_manager.subscribe("user_input", listener_a)
        event_manager.subscribe("data_ready", listener_b)

        await event_manager.publish("user_input", {"message": "Olá GEM!"})
        ...
        await event_manager.publish("non_existent_event", {"info": "Este não deve ser ouvido por ninguém."})
        ...
        # Publicar novamente para ver se o Listener C ainda é invocado
        print("\n--- Publicando 'status_update' novamente após desubscrição do Listener C ---")
        ...
        print("✅ Listener C verificado como desubscrito.")

        print("\n--- Teste 4: Desligamento do EventManager ---")
        event_manager.shutdown()
        # Após o desligamento, a publicação não deve invocar nada
        await event_manager.publish("user_input", {"message": "Isto não deve ser ouvido após o desligamento."})
        await asyncio.sleep(0.1)
        assert not event_manager._listeners # Verifica se os listeners foram limpos
        print("✅ EventManager desligado e listeners limpos.")

        print("\n--- Testes do EventManager concluídos com sucesso. ---")
