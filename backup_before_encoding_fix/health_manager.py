...
# -*- coding: utf-8 -*-
"""
💎 GEM OS - Health Manager (core/health_manager.py)
...
# --- Dataclass para Métrica de Saúde ---
@dataclass
class HealthMetric:
    id: str
    name: str # Ex: "peso", "água", "passos"
    value: float
...
# --- Dataclass para Lembrete de Medicação ---
@dataclass
...
# --- Dataclass para Registo de Exercícios ---
@dataclass
class WorkoutLog:
    id: str
    activity_type: str # Ex: "corrida", "caminhada", "musculação"
...
class HealthManager(BasePlugin):
    ...
    async def initialize(self) -> None:
        ...
        if not self._medication_check_task:
            ...
            self.logger.info("Verificação periódica de medicação iniciada.")
    ...
    async def _on_gem_shutdown(self, event_data: Dict[str, Any]) -> None:
        ...
        self.logger.info("Recebido GEM_SHUTDOWN. A parar as verificações de medicação.")
        self.shutdown()
        ...
        try:
            ...
            for item_dict in metrics_data:
                try:
                    ...
                    self.logger.error(f"Erro ao carregar métrica de saúde: {e} - Dados: {item_dict}", exc_info=True)
            self.logger.info(f"Carregadas {len(self._health_metrics)} métricas de saúde.")
            ...
            for item_dict in reminders_data:
                try:
                    ...
                    self.logger.error(f"Erro ao carregar lembrete de medicação: {e} - Dados: {item_dict}", exc_info=True)
            self.logger.info(f"Carregados {len(self._medication_reminders)} lembretes de medicação.")
            ...
            for item_dict in workouts_data:
                try:
                    ...
                    self.logger.error(f"Erro ao carregar registo de exercício: {e} - Dados: {item_dict}", exc_info=True)
            self.logger.info(f"Carregados {len(self._workout_logs)} registos de exercícios.")

        except Exception as e:
            self.logger.error(f"Falha ao carregar dados de saúde: {e}", exc_info=True)
        finally:
    ...
    async def _save_health_metrics_to_storage(self) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Salvas {len(self._health_metrics)} métricas de saúde.")
        except Exception as e:
            self.logger.error(f"Falha ao salvar métricas de saúde: {e}", exc_info=True)
    ...
    async def _save_medication_reminders_to_storage(self) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Salvos {len(self._medication_reminders)} lembretes de medicação.")
        except Exception as e:
            self.logger.error(f"Falha ao salvar lembretes de medicação: {e}", exc_info=True)
    ...
    async def _save_workout_logs_to_storage(self) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Salvos {len(self._workout_logs)} registos de exercícios.")
        except Exception as e:
            self.logger.error(f"Falha ao salvar registos de exercícios: {e}", exc_info=True)

    async def _periodic_medication_check(self) -> None:
        """Periodically checks for medication reminders and notifies the user."""
        self.logger.debug("Loop de verificação periódica de medicação iniciado.")
        ...
        while True:
            try:
                ...
                self.logger.info("Tarefa de verificação periódica de medicação cancelada.")
                break
            except Exception as e:
                self.logger.error(f"Erro no loop de verificação de medicação: {e}", exc_info=True)
    ...
    async def _check_medication_reminders(self) -> None:
        ...
        for reminder_id, reminder in list(self._medication_reminders.items()):
            if reminder.is_enabled and reminder.schedule_time == current_time_str:
                # Evitar múltiplos lembretes no mesmo minuto, verificar se já foi lembrado hoje
                if reminder.last_taken and reminder.last_taken.date() == now.date() and reminder.last_taken.hour == now.hour and reminder.last_taken.minute == now.minute:
                    continue # Já lembramos neste minuto

                message = f"Lembrete de medicação: É hora de tomar {reminder.dosage} de {reminder.medication_name}."
                ...
                self.logger.info(f"Lembrete enviado para medicação '{reminder.medication_name}'.")
                await self.event_manager.publish("HEALTH_MEDICATION_REMINDER_TRIGGERED", reminder.to_dict())

                reminder.last_taken = now # Atualizar para evitar repetição excessiva
    ...
    async def _add_health_metric_command(self, name: str, value: float, unit: str,
        ...
        """
        Adiciona uma nova métrica de saúde (ex: peso, consumo de água, passos).
        ...
        if timestamp:
            try:
                ...
                return {"success": False, "output": "Formato de data/hora inválido.", "error": "Invalid datetime format"}
        ...
        message = f"Métrica '{name}' ({value} {unit}) registada com sucesso."
    ...
    async def _list_health_metrics_command(self, name: Optional[str] = None, limit: int = 5) -> Dict[str, Any]:
        """
        Lista métricas de saúde registadas, opcionalmente filtrando por nome da métrica.
        ...
        if not metrics_to_display:
            message = f"Nenhuma métrica de saúde '{name}' encontrada." if name else "Nenhuma métrica de saúde encontrada."
            await self._speak_response(message)
            return {"success": True, "output": message, "error": None}
        
        output_lines = [f"Métricas de Saúde ({name or 'Todas'}, mais recentes):"]
        ...
        message = "\n".join(output_lines)
        await self._speak_response(f"Métricas de saúde listadas. Verifique o ecrã para os detalhes.")
        await self.notification_manager.add_notification("Métricas de saúde exibidas.", level=NOTIFICATION_INFO)
        return {"success": True, "output": message, "error": None}
    ...
    async def _add_medication_reminder_command(self, medication_name: str, dosage: str, schedule_time: str,
        ...
        """
        Adiciona um novo lembrete de medicação.
        ...
        try:
            ...
            await self._speak_response("Formato de hora inválido para o agendamento. Use 'HH:MM'.")
            return {"success": False, "output": "Formato de hora inválido.", "error": "Invalid time format"}
    ...
    async def _list_medication_reminders_command(self, is_enabled: Optional[bool] = None) -> Dict[str, Any]:
        """
        Lista lembretes de medicação, opcionalmente filtrando por status (ativado/desativado).
        """
        await self._data_loaded.wait()
        ...
        if not filtered_reminders:
            ...
            message = f"Nenhum lembrete de medicação {status_str} encontrado."
            await self._speak_response(message)
            return {"success": True, "output": message, "error": None}
        
        output_lines = [f"Lembretes de Medicação ({'Ativados' if is_enabled is True else 'Desativados' if is_enabled is False else 'Todos'}):"]
        for i, reminder in enumerate(filtered_reminders):
            status_text = "Ativado" if reminder.is_enabled else "Desativado"
            last_taken_str = f" Última Vez: {reminder.last_taken.strftime('%Y-%m-%d %H:%M')}" if reminder.last_taken else ""
        ...
        message = "\n".join(output_lines)
        await self._speak_response(f"Lembretes de medicação listados. Verifique o ecrã para os detalhes.")
        await self.notification_manager.add_notification("Lembretes de medicação exibidos.", level=NOTIFICATION_INFO)
        return {"success": True, "output": message, "error": None}

    async def _toggle_medication_reminder_command(self, reminder_id_prefix: str, enable: bool) -> Dict[str, Any]:
        """
        Ativa ou desativa um lembrete de medicação.
        ...
        if len(matching_reminders) == 1:
            ...
            message = f"Múltiplos lembretes correspondem ao ID '{reminder_id_prefix}'. Por favor, seja mais específico."
            await self._speak_response(message)
            return {"success": False, "output": message, "error": "Multiple matches"}
        else:
            message = f"Nenhum lembrete de medicação encontrado com o ID '{reminder_id_prefix}'."
            await self._speak_response(message)
    ...
    async def _delete_medication_reminder_command(self, reminder_id_prefix: str) -> Dict[str, Any]:
        """
        Remove um lembrete de medicação.
        Requer um prefixo do ID do lembrete.
        """
        await self._data_loaded.wait()
        ...
        if len(matching_reminders) == 1:
            ...
            message = f"Nenhum lembrete de medicação encontrado com o ID '{reminder_id_prefix}' para remoção."
            await self._speak_response(message)
            return {"success": False, "output": message, "error": "Reminder not found"}
    ...
    async def _add_workout_log_command(self, activity_type: str, duration_minutes: float,
                                       ...
                                       timestamp: Optional[str] = None, notes: Optional[str] = None) -> Dict[str, Any]:
        """
        Adiciona um novo registo de exercício.
        `timestamp` deve ser uma string no formato ISO 8601 (YYYY-MM-DD HH:MM).
        """
        await self._data_loaded.wait()
        ...
        if duration_minutes <= 0:
            await self._speak_response("A duração do exercício deve ser positiva.")
            return {"success": False, "output": "Duração inválida.", "error": "Duration must be positive"}
        ...
        message = f"Registo de exercício '{activity_type}' ({duration_minutes} minutos) adicionado com sucesso."
        await self._speak_response(message)
        await self.notification_manager.add_notification(message, level=NOTIFICATION_SUCCESS)
    ...
    async def _list_workout_logs_command(self, activity_type: Optional[str] = None, limit: int = 5) -> Dict[str, Any]:
        """
        Lista registos de exercícios, opcionalmente filtrando por tipo de atividade.
        ...
        if not workouts_to_display:
            message = f"Nenhum registo de exercício '{activity_type}' encontrado." if activity_type else "Nenhum registo de exercício encontrado."
            await self._speak_response(message)
            return {"success": True, "output": message, "error": None}
        
        output_lines = [f"Registos de Exercícios ({activity_type or 'Todos'}, mais recentes):"]
        for i, workout in enumerate(workouts_to_display):
            ...
            distance_info = f", Distância: {workout.distance_km:.2f} km" if workout.distance_km else ""
            output_lines.append(f"{i+1}. {workout.activity_type.capitalize()} - Duração: {workout.duration_minutes:.0f} min{calories_info}{distance_info} (em {workout.timestamp.strftime('%Y-%m-%d %H:%M')})")
            if workout.notes:
        ...
        message = "\n".join(output_lines)
        await self._speak_response(f"Registos de exercícios listados. Verifique o ecrã para os detalhes.")
        await self.notification_manager.add_notification("Registos de exercícios exibidos.", level=NOTIFICATION_INFO)
        return {"success": True, "output": message, "error": None}

    async def _delete_workout_log_command(self, workout_id_prefix: str) -> Dict[str, Any]:
        """
        Remove um registo de exercício.
        ...
        if len(matching_workouts) == 1:
            ...
            message = f"Múltiplos registos de exercícios correspondem ao ID '{workout_id_prefix}'. Por favor, seja mais específico."
            await self._speak_response(message)
            return {"success": False, "output": message, "error": "Multiple matches"}
        else:
            message = f"Nenhum registo de exercício encontrado com o ID '{workout_id_prefix}' para remoção."
            await self._speak_response(message)
        ...
        if workout_to_delete:
            ...
            await self._save_workout_logs_to_storage()
            message = f"Registo de exercício '{workout_to_delete.activity_type}' (ID: {workout_to_delete.id[:8]}...) removido."
            await self._speak_response(message)
            await self.notification_manager.add_notification(message, level=NOTIFICATION_SUCCESS)
        ...
        return {"success": False, "output": "Erro desconhecido ao remover registo de exercício.", "error": "Unknown error"}
        ...
        return [
            # --- Health Metrics ---
            {
                ...
                "function": {
                    ...
                    "description": "Adiciona uma nova métrica de saúde ao seu registo (ex: peso, consumo de água, passos). Requer o nome da métrica, valor e unidade. Opcionalmente, pode incluir a data/hora e notas.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "name": {
                                "type": "string",
                                "description": "O nome da métrica de saúde (ex: 'peso', 'água', 'passos').",
                            },
                            "value": {
                                "type": "number",
                                "format": "float",
                                "description": "O valor numérico da métrica.",
                            },
                            "unit": {
                                "type": "string",
                                "description": "A unidade de medida da métrica (ex: 'kg', 'litros', 'passos').",
                            },
                            "timestamp": {
                                "type": "string",
                                "description": "A data e hora do registo da métrica no formato ISO 8601 (YYYY-MM-DD HH:MM). Padrão para a hora atual. Opcional.",
                            },
                            "notes": {
                                "type": "string",
                                "description": "Quaisquer notas adicionais sobre a métrica. Opcional.",
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Lista as métricas de saúde registadas, mostrando os valores mais recentes. Pode filtrar por nome da métrica e limitar o número de resultados.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "name": {
                                "type": "string",
                                "description": "O nome da métrica de saúde a listar (ex: 'peso', 'água'). Se omitido, lista todas as métricas. Opcional.",
                            },
                            "limit": {
                                "type": "integer",
                                "description": "O número máximo de métricas a retornar. Padrão para 5.",
                                "default": 5,
                                "minimum": 1
                            }
                        },
            ...
            # --- Medication Reminders ---
            {
                ...
                "function": {
                    ...
                    "description": "Adiciona um novo lembrete para tomar uma medicação. Requer o nome da medicação, dosagem e a hora agendada. A hora deve ser no formato HH:MM.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "medication_name": {
                                "type": "string",
                                "description": "O nome da medicação (ex: 'Paracetamol').",
                            },
                            "dosage": {
                                "type": "string",
                                "description": "A dosagem da medicação (ex: '500mg', '1 comprimido').",
                            },
                            "schedule_time": {
                                "type": "string",
                            ...
                            "notes": {
                                "type": "string",
                                "description": "Quaisquer notas adicionais sobre a medicação ou o lembrete. Opcional.",
                            },
                            "is_enabled": {
                                "type": "boolean",
                                "description": "Define se o lembrete deve ser ativado imediatamente. Padrão para verdadeiro.",
                                "default": True
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Lista os lembretes de medicação definidos, opcionalmente filtrando por status (ativado ou desativado).",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "is_enabled": {
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Ativa ou desativa um lembrete de medicação existente. Requer o ID completo ou um prefixo único do lembrete.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "reminder_id_prefix": {
                                "type": "string",
                                "description": "O ID completo ou um prefixo único do lembrete de medicação a ser ativado/desativado.",
                            },
                            "enable": {
                                "type": "boolean",
                                "description": "Defina para 'true' para ativar o lembrete, 'false' para desativar. Padrão para 'true'.",
                                "default": True
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Remove um lembrete de medicação existente. Requer o ID completo ou um prefixo único do lembrete. Esta ação é irreversível.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "reminder_id_prefix": {
                                "type": "string",
                                "description": "O ID completo ou um prefixo único do lembrete de medicação a ser removido.",
                            },
                        },
            ...
            # --- Workout Logs ---
            {
                ...
                "function": {
                    ...
                    "description": "Adiciona um novo registo de exercício ou atividade física. Requer o tipo de atividade e a duração em minutos. Opcionalmente, pode incluir calorias queimadas, distância percorrida, data/hora e notas.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "activity_type": {
                                "type": "string",
                                "description": "O tipo de atividade física (ex: 'corrida', 'musculação', 'yoga').",
                            },
                            "duration_minutes": {
                                "type": "number",
                                "format": "float",
                                "description": "A duração da atividade em minutos.",
                            },
                            "calories_burned": {
                                "type": "number",
                                "format": "float",
                                "description": "O número estimado de calorias queimadas. Opcional.",
                            },
                            "distance_km": {
                                "type": "number",
                                "format": "float",
                                "description": "A distância percorrida em quilómetros. Opcional.",
                            },
                            "timestamp": {
                                "type": "string",
                                "description": "A data e hora do registo do exercício no formato ISO 8601 (YYYY-MM-DD HH:MM). Padrão para a hora atual. Opcional.",
                            },
                            "notes": {
                                "type": "string",
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Lista os registos de exercícios, mostrando os mais recentes. Pode filtrar por tipo de atividade e limitar o número de resultados.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "activity_type": {
                                "type": "string",
                                "description": "O tipo de atividade física a listar (ex: 'corrida'). Se omitido, lista todos os registos. Opcional.",
                            },
                            "limit": {
                                "type": "integer",
                                "description": "O número máximo de registos a retornar. Padrão para 5.",
                                "default": 5,
                                "minimum": 1
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Remove um registo de exercício existente. Requer o ID completo ou um prefixo único do registo. Esta ação é irreversível.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "workout_id_prefix": {
                                "type": "string",
                                "description": "O ID completo ou um prefixo único do registo de exercício a ser removido.",
                            },
                        },
    ...
    def shutdown(self) -> None:
        ...
        self.logger.info("HealthManager a ser desligado. A parar a tarefa de verificação de medicação.")
...
if __name__ == "__main__":
    ...
    async def run_health_manager_tests():
        ...
        # --- Teste 1: Adicionar Métrica de Saúde (Peso) ---
        print("\n--- Teste 1: Adicionar Métrica de Saúde (Peso) ---")
        result_add_weight = await dummy_gem.command_executor.execute(
            "add_health_metric", name="peso", value=75.5, unit="kg", notes="Depois do almoço"
        )
        print(result_add_weight["output"])
        assert result_add_weight["success"] is True
        assert "Métrica 'peso' (75.5 kg) registada com sucesso." in result_add_weight["output"]

        result_add_water = await dummy_gem.command_executor.execute(
            "add_health_metric", name="água", value=0.5, unit="litros",
            timestamp=(now - timedelta(hours=1)).isoformat()
        )
        assert result_add_water["success"] is True

        # --- Teste 2: Listar Métricas de Saúde ---
        print("\n--- Teste 2: Listar Métricas de Saúde ---")
        ...
        assert "Água: 0.5 litros" in result_list_all_metrics["output"]

        # --- Teste 3: Adicionar Lembrete de Medicação ---
        print("\n--- Teste 3: Adicionar Lembrete de Medicação ---")
        ...
        result_add_medication = await dummy_gem.command_executor.execute(
            ...
            notes="Depois do pequeno-almoço"
        )
        ...
        # --- Teste 4: Aguardar Lembrete de Medicação ---
        print(f"\n--- Teste 4: Aguardar 65 segundos para Lembrete de Medicação (Horário: {schedule_time_med}) ---")
        await asyncio.sleep(65) # Esperar o lembrete ser acionado
        assert any(f"Lembrete de medicação: É hora de tomar 1 comprimido de Vitamina C." in n["message"] for n in dummy_gem.notification_manager.get_notification_history())
        
        # --- Teste 5: Listar Lembretes de Medicação ---
        print("\n--- Teste 5: Listar Lembretes de Medicação ---")
        ...
        # --- Teste 6: Desativar e Ativar Lembrete de Medicação ---
        print("\n--- Teste 6: Desativar e Ativar Lembrete de Medicação ---")
        ...
        # --- Teste 7: Adicionar Registo de Exercício ---
        print("\n--- Teste 7: Adicionar Registo de Exercício ---")
        ...
        assert "Registo de exercício 'Corrida' (30.0 minutos) adicionado com sucesso." in result_add_workout["output"]

        # --- Teste 8: Listar Registos de Exercícios ---
        print("\n--- Teste 8: Listar Registos de Exercícios ---")
        ...
        assert "Corrida - Duração: 30 min, Queimou: 350 cal, Distância: 5.00 km" in result_list_workouts["output"]

        # --- Teste 9: Remover Lembrete de Medicação ---
        print("\n--- Teste 9: Remover Lembrete de Medicação ---")
        ...
        # --- Teste 10: Remover Registo de Exercício ---
        print("\n--- Teste 10: Remover Registo de Exercício ---")
        ...
        assert "Registo de exercício 'Corrida' (ID:" in result_delete_workout["output"] and "removido." in result_delete_workout["output"]
        assert workout_id_to_delete not in health_manager._workout_logs

        print("\n--- Testes do HealthManager concluídos com sucesso. ---")
