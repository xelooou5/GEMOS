...
# -*- coding: utf-8 -*-
"""
💎 GEM OS - Web Search Manager (core/web_search_manager.py)
...
class WebSearchManager(BasePlugin):
    ...
    def __init__(self, gem_instance: GEMVoiceAssistant, logger: Optional[logging.Logger] = None):
        ...
        if not self.search_api_key:
            self.logger.warning("SEARCH_API_KEY não configurado. A pesquisa web pode não funcionar.")
        if not self.search_engine_id:
            self.logger.warning("SEARCH_ENGINE_ID não configurado. A pesquisa web pode não funcionar.")
    ...
    async def _perform_web_search(self, query: str, num_results: int = 5) -> List[SearchResult]:
        ...
        if not self.search_api_key or not self.search_engine_id:
            self.logger.error("API Key ou Search Engine ID ausentes. Não é possível realizar pesquisa.")
            await self._speak_response("Desculpe, a pesquisa web não está configurada corretamente.")
        ...
        try:
            ...
            self.logger.info(f"Pesquisa web para '{query}' concluída. Encontrados {len(results)} resultados.")
            await self.notification_manager.add_notification(
                f"Pesquisa web concluída para '{query}'.", level=NOTIFICATION_SUCCESS, vocalize=False
            )
            ...
            error_msg = f"Erro de rede/requisição ao realizar pesquisa web para '{query}': {e}"
            self.logger.error(error_msg, exc_info=True)
    ...
    async def _search_web_command(self, query: str, num_results: int = 3) -> Dict[str, Any]:
        ...
        """
        if not query:
            message = "A consulta de pesquisa não pode estar vazia."
        ...
        if not results:
            message = f"Não foram encontrados resultados para '{query}'."
            await self._speak_response(message)
        ...
        response_text = "\n".join(output_lines)
        await self._speak_response(f"Foram encontrados resultados para '{query}'. Verifique o ecrã para os detalhes.")
        ...
        return [
            {
                ...
                "function": {
                    "name": "search_web",
                    "description": "Realiza uma pesquisa na web para uma determinada consulta e retorna os resultados mais relevantes. Use para responder a perguntas sobre factos, informações atuais ou conhecimento geral que não estejam diretamente no seu conhecimento.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "query": {
                            ...
                            "num_results": {
                                "type": "integer",
                                "description": "O número máximo de resultados de pesquisa a retornar. Padrão para 3.",
                                "default": 3
                            }
                        },
                        "required": ["query"],
...
if __name__ == "__main__":
    ...
    async def run_web_search_manager_tests():
        ...
        # Configurar variáveis de ambiente para o teste, se necessário, ou usar as dummy keys
        # Lembre-se que para um teste real, você precisará de uma chave de API e um ID de motor de pesquisa válidos.
        if not dummy_gem.config_manager.get_config().general.search_api_key == "YOUR_GOOGLE_SEARCH_API_KEY":
            ...
            print("\n--- Teste 2: Pesquisar na Web (múltiplos resultados) ---")
            result_search_multi = await dummy_gem.command_executor.execute("search_web", query="últimas notícias de tecnologia", num_results=2)
            print(result_search_multi["output"])
            assert result_search_multi["success"] is True
            assert "Resultados da pesquisa para 'últimas notícias de tecnologia':" in result_search_multi["output"]

            print("\n--- Teste 3: Pesquisar na Web (sem resultados esperados) ---")
            result_no_results = await dummy_gem.command_executor.execute("search_web", query="asjdlfkasjdfaslkhflas", num_results=1) # Consulta aleatória
            print(result_no_results["output"])
            assert result_no_results["success"] is True
            assert "Não foram encontrados resultados para 'asjdlfkasjdfaslkhflas'." in result_no_results["output"]
        else:
            logger.warning("SEARCH_API_KEY ou SEARCH_ENGINE_ID não configurados (usando valores dummy). Testes de pesquisa web reais serão ignorados.")
            logger.warning("Para executar testes de pesquisa web reais, defina as variáveis de ambiente SEARCH_API_KEY e SEARCH_ENGINE_ID.")
        ...
        assert "A consulta de pesquisa não pode estar vazia." in result_empty_query["error"]


        print("\n--- Testes do WebSearchManager concluídos com sucesso. ---")
