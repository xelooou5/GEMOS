...
# -*- coding: utf-8 -*-
"""
💎 GEM OS - Automation Manager (core/automation_manager.py)
...
# --- Dataclass para Rotina de Automação ---
@dataclass
...
class AutomationManager(BasePlugin):
    ...
    async def _on_gem_shutdown(self, event_data: Dict[str, Any]) -> None:
        ...
        self.logger.info("Recebido GEM_SHUTDOWN. A desligar o agendador de automação.")
        self.shutdown()
    ...
    async def _load_routines_from_storage(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Carregadas {len(self._routines)} rotinas de automação do armazenamento.")
        except Exception as e:
    ...
    def _start_scheduler(self) -> None:
        ...
        if not self._is_scheduler_running:
            ...
            self.logger.info("Agendador de automação em segundo plano iniciado.")

    def _run_scheduler_loop(self) -> None:
        """The main loop for the schedule library."""
        self.logger.debug("Loop do agendador de automação iniciado.")
        while self._is_scheduler_running:
            try:
                schedule.run_pending()
                time.sleep(1) # Check every second
            except Exception as e:
                self.logger.error(f"Erro no loop do agendador de automação: {e}", exc_info=True)
                time.sleep(5) # Wait before retrying
        self.logger.debug("Loop do agendador de automação terminado.")
    ...
    def _schedule_routine(self, routine: AutomationRoutine) -> None:
        ...
        if not routine.is_enabled:
            self.logger.debug(f"Rotina '{routine.name}' desativada, não será agendada.")
            return
        ...
        if routine.trigger_type == "time_daily" and routine.trigger_value:
            ...
            self.logger.info(f"Rotina diária '{routine.name}' agendada para {routine.trigger_value}.")
            ...
            try:
                ...
                if trigger_dt > datetime.now():
                    # Calculamos o atraso e agendamos uma única vez
                    delay_seconds = (trigger_dt - datetime.now()).total_seconds()
                    if delay_seconds > 0:
                        self.logger.info(f"Rotina única '{routine.name}' agendada para daqui a {delay_seconds:.0f} segundos.")
                        ...
                        self.logger.warning(f"Rotina única '{routine.name}' com tempo de gatilho no passado. Ignorando.")
                else:
                     self.logger.warning(f"Rotina única '{routine.name}' com tempo de gatilho no passado. Ignorando.")
            except ValueError:
                self.logger.error(f"Formato de data/hora inválido para rotina única '{routine.name}': {routine.trigger_value}")
            ...
            self.logger.warning(f"Tipo de gatilho de rotina desconhecido ou inválido para '{routine.name}': {routine.trigger_type}, Valor: {routine.trigger_value}")
    ...
    async def _execute_routine_actions(self, routine: AutomationRoutine) -> None:
        ...
        for action in routine.actions:
            self.logger.debug(f"Executando ação: {action.command_name} com args: {action.args}")
            try:
                result = await self.command_executor.execute(action.command_name, **(action.args or {}))
                if not result.get("success"):
                    self.logger.error(f"Ação '{action.command_name}' na rotina '{routine.name}' falhou: {result.get('error')}")
                    await self.notification_manager.add_notification(
                        f"Ação '{action.command_name}' na rotina '{routine.name}' falhou: {result.get('error')}", level=NOTIFICATION_ERROR
                    )
                    await self.tts_module.speak(f"Falha na rotina {routine.name}, ação {action.command_name}.")
                    # Decidir se deve parar a rotina ou continuar com outras ações
                    # Por enquanto, continuamos, mas podemos adicionar uma flag na rotina
            except Exception as e:
                self.logger.error(f"Erro inesperado ao executar ação '{action.command_name}' na rotina '{routine.name}': {e}", exc_info=True)
                await self.notification_manager.add_notification(
                    f"Erro inesperado na ação '{action.command_name}' na rotina '{routine.name}': {e}", level=NOTIFICATION_ERROR
                )
        ...
        self.logger.info(f"Rotina '{routine.name}' concluída.")
        await self.notification_manager.add_notification(
            f"Rotina '{routine.name}' concluída com sucesso.", level=NOTIFICATION_SUCCESS, vocalize=False
        )

    async def _execute_routine_actions_once(self, routine: AutomationRoutine) -> None:
        """Executa as ações de uma rotina única vez e a remove do agendador."""
        await self._execute_routine_actions(routine)
        schedule.clear(routine.id) # Remove o job agendado
        self.logger.info(f"Rotina única '{routine.name}' executada e removida do agendador.")
        # Se for uma rotina time_once, podemos desativá-la ou excluí-la após a execução
        routine.is_enabled = False # Desativa após uma execução
        await self._save_routines_to_storage()
    ...
    def _clear_routine_schedule(self, routine_id: str) -> None:
        ...
        # Também desinscreve de eventos se for o caso
    ...
    async def _create_automation_routine_command(self, name: str, trigger_type: str, trigger_value: Optional[str],
        ...
        """
        Cria uma nova rotina de automação.
        ...
        try:
            ...
            await self._speak_response(f"Formato JSON de ações inválido: {e}. Certifique-se de que é uma lista de objetos de ação válidos.")
            return {"success": False, "output": "Formato de ações inválido.", "error": str(e)}

        if not actions:
            await self._speak_response("Uma rotina deve ter pelo menos uma ação.")
            return {"success": False, "output": "Rotina sem ações.", "error": "Routine must have at least one action"}
        ...
        message = f"Rotina de automação '{name}' criada com sucesso."
    ...
    async def _list_automation_routines_command(self) -> Dict[str, Any]:
        """
        Lista todas as rotinas de automação definidas.
        """
        await self._routines_loaded.wait()

        if not self._routines:
            message = "Nenhuma rotina de automação definida."
            await self._speak_response(message)
            return {"success": True, "output": message, "error": None}
        
        output_lines = ["Rotinas de Automação:"]
        for i, routine in enumerate(self._routines.values()):
            ...
            last_triggered_info = f" Último Acionamento: {routine.last_triggered.strftime('%Y-%m-%d %H:%M')}" if routine.last_triggered else ""
            ...
            for j, action in enumerate(routine.actions):
                output_lines.append(f"   Ação {j+1}: {action.command_name} {action.args or ''}")
            output_lines.append("-" * 20)
        
        message = "\n".join(output_lines)
        await self._speak_response(f"As suas rotinas de automação foram listadas. Verifique o ecrã para os detalhes.")
        await self.notification_manager.add_notification("Lista de rotinas de automação exibida.", level=NOTIFICATION_INFO)
        return {"success": True, "output": message, "error": None}

    async def _toggle_automation_routine_command(self, routine_id_prefix: str, enable: bool) -> Dict[str, Any]:
        """
        Ativa ou desativa uma rotina de automação.
        """
        await self._routines_loaded.wait()
        ...
        if len(matching_routines) == 1:
            ...
            message = f"Múltiplas rotinas correspondem ao ID '{routine_id_prefix}'. Por favor, seja mais específico."
            await self._speak_response(message)
    ...
    async def _delete_automation_routine_command(self, routine_id_prefix: str) -> Dict[str, Any]:
        """
        Remove uma rotina de automação.
        """
        await self._routines_loaded.wait()
        ...
        if len(matching_routines) == 1:
            ...
            message = f"Nenhuma rotina encontrada com o ID '{routine_id_prefix}' para remoção."
            await self._speak_response(message)
        ...
        return [
            {
                ...
                "function": {
                    ...
                    "description": "Cria uma nova rotina de automação com um nome, tipo de gatilho (time_daily, time_once, event_based) e uma lista de ações a executar. As ações devem ser fornecidas como uma string JSON de objetos de ação.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "name": {
                            ...
                            "trigger_type": {
                                "type": "string",
                                "description": "O tipo de gatilho para a rotina. 'time_daily' para gatilhos diários, 'time_once' para gatilhos únicos, 'event_based' para gatilhos por evento do sistema.",
                                "enum": ["time_daily", "time_once", "event_based"]
                            },
                            "trigger_value": {
                                "type": "string",
                                "description": "O valor associado ao gatilho. Para 'time_daily', é a hora (HH:MM). Para 'time_once', é a data e hora (YYYY-MM-DD HH:MM). Para 'event_based', é o nome do evento (ex: 'NEW_EMAIL_RECEIVED').",
                            },
                            "actions_json": {
                                "type": "string",
                                "description": "Uma string JSON que representa uma lista de ações a serem executadas. Cada ação é um objeto com 'command_name' e 'args' (opcional). Ex: '[{\"command_name\": \"tell_time\"}, {\"command_name\": \"speak_text\", \"args\": {\"text\": \"Hora atual.\"}}]'",
                            },
                            "is_enabled": {
                                "type": "boolean",
                                "description": "Define se a rotina deve ser ativada imediatamente. Padrão para verdadeiro.",
                                "default": True
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Lista todas as rotinas de automação que foram definidas, mostrando os seus nomes, status, gatilhos e ações.",
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Ativa ou desativa uma rotina de automação existente. Requer o ID completo ou um prefixo único da rotina.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "routine_id_prefix": {
                                "type": "string",
                                "description": "O ID completo ou um prefixo único da rotina a ser ativada/desativada.",
                            },
                            "enable": {
                                "type": "boolean",
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Remove uma rotina de automação existente. Requer o ID completo ou um prefixo único da rotina. Esta ação é irreversível.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "routine_id_prefix": {
                                "type": "string",
                                "description": "O ID completo ou um prefixo único da rotina a ser removida.",
                            },
                        },
...
if __name__ == "__main__":
    ...
    class DummyCommandExecutor:
        ...
        async def execute(self, command_name: str, **kwargs) -> Dict[str, Any]:
            ...
            if command_name == "tell_time":
                return {"success": True, "output": f"A hora atual é {datetime.now().strftime('%H:%M')}", "error": None}
    ...
    async def run_automation_manager_tests():
        ...
        daily_actions_json = json.dumps([
            ...
            {"command_name": "speak_text", "args": {"text": "Bom dia, é hora de começar o dia!"}}
        ])
        ...
        assert "Rotina de automação 'Rotina de Bom Dia' criada com sucesso." in result_create_daily["output"]
        ...
        assert any("speak_text" == cmd[0] and "Bom dia, é hora de começar o dia!" in cmd[1].get("text", "") for cmd in dummy_gem.command_executor.executed_commands)
        ...
        once_actions_json = json.dumps([
            ...
            {"command_name": "speak_text", "args": {"text": "Verificação única do sistema concluída."}}
        ])
        result_create_once = await dummy_gem.command_executor.execute(
            "create_automation_routine",
            name="Verificação Única do Sistema",
        ...
        print(f"\n--- Aguardando {16} segundos para a Verificação Única do Sistema (Trigger: {trigger_time_once}) ---")
        ...
        assert any("speak_text" == cmd[0] and "Verificação única do sistema concluída." in cmd[1].get("text", "") for cmd in dummy_gem.command_executor.executed_commands)
        ...
        # Check if "Verificação Única do Sistema" is listed and is_enabled is False
        assert "Verificação Única do Sistema (Desativada)" in result_list_after_once["output"]
        ...
        print("\n--- Testes do AutomationManager concluídos com sucesso. ---")
