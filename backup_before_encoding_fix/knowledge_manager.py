...
# -*- coding: utf-8 -*-
"""
💎 GEM OS - Knowledge Manager (core/knowledge_manager.py)
...
class KnowledgeManager(BasePlugin):
    ...
    async def _add_knowledge_command(self, key: str, value: str, source: str = "user_input") -> Dict[str, Any]:
        ...
        Args:
            key: A chave ou nome do conhecimento (ex: "meu_endereço", "minha_comida_favorita").
            value: O valor ou conteúdo do conhecimento.
    ...
    async def _get_knowledge_command(self, key: str) -> Dict[str, Any]:
        ...
        if not entry:
            message = f"Conhecimento '{key}' não encontrado."
            await self._speak_response(message)
            return {"success": False, "output": "", "error": message}
        
        message = f"O conhecimento para '{key}' é: {entry.value}"
        await self._speak_response(message)
    ...
    async def _delete_knowledge_command(self, key: str) -> Dict[str, Any]:
        ...
        if key not in self._knowledge:
            message = f"Conhecimento '{key}' não encontrado para remoção."
            await self._speak_response(message)
            return {"success": False, "output": "", "error": message}
    ...
    async def _list_knowledge_command(self) -> Dict[str, Any]:
        ...
        message = "\n".join(output_lines)
        await self._speak_response(f"As suas entradas de conhecimento foram listadas. Verifique o ecrã para as chaves.")
        ...
        return [
            {
                ...
                "function": {
                    "name": "add_knowledge",
                    "description": "Adiciona ou atualiza uma informação específica (facto) que o GEM deve lembrar. Isso permite ao GEM aprender e personalizar as suas respostas com base no conhecimento fornecido pelo usuário.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "key": {
                                "type": "string",
                                "description": "Uma chave ou nome único para a informação (ex: 'meu_endereço', 'comida_favorita', 'nome_do_animal_de_estimacao').",
                            },
                            "value": {
                                "type": "string",
                                "description": "O valor ou conteúdo da informação a ser armazenada (ex: 'Rua Principal, 123', 'pizza', 'Max').",
                            },
                            "source": {
                                "type": "string",
                                "description": "A origem do conhecimento. Padrão para 'user_input'.",
                                "default": "user_input"
                            }
                        },
                        "required": ["key", "value"],
            ...
            },
            {
                ...
                "function": {
                    "name": "get_knowledge",
                    "description": "Recupera uma informação específica (facto) que o GEM aprendeu, utilizando a chave ou nome da informação. Use para responder a perguntas que dependem do conhecimento prévio que o usuário forneceu.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "key": {
                                "type": "string",
                                "description": "A chave ou nome da informação a ser recuperada.",
                            },
                        },
                        "required": ["key"],
                    },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Remove uma entrada de conhecimento armazenada pelo seu nome ou chave. Esta ação é irreversível.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "key": {
                                "type": "string",
                                "description": "A chave ou nome da informação a ser removida.",
                            },
                        },
                        "required": ["key"],
                    },
            ...
            },
            {
                ...
                "function": {
                    "name": "list_knowledge",
                    "description": "Lista todas as chaves (nomes) das informações que o GEM aprendeu e tem armazenadas. Não revela os valores das informações por padrão.",
...
if __name__ == "__main__":
    ...
    async def run_knowledge_manager_tests():
        ...
        assert "Conhecimento 'comida_favorita' não encontrado." in result_get_invalid["error"]
        ...
        print("\n--- Testes do KnowledgeManager concluídos com sucesso. ---")
