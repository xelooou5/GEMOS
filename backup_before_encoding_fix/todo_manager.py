...
# -*- coding: utf-8 -*-
"""
💎 GEM OS - To-Do Manager (core/todo_manager.py)
...
class TodoManager(BasePlugin):
    ...
    async def initialize(self) -> None:
        ...
        if not self._check_task:
            ...
            self.logger.info("Verificação periódica de tarefas iniciada.")
    ...
    async def _on_gem_shutdown(self, event_data: Dict[str, Any]) -> None:
        ...
        self.logger.info("Recebido GEM_SHUTDOWN. A parar as verificações de tarefas.")
        self.shutdown()
    ...
    async def _periodic_task_check(self) -> None:
        ...
        self.logger.debug("Loop de verificação periódica de tarefas iniciado.")
        ...
        while True:
            try:
                ...
                self.logger.info("Tarefa de verificação periódica de tarefas cancelada.")
                break
            except Exception as e:
                self.logger.error(f"Erro no loop de verificação de tarefas: {e}", exc_info=True)
            ...
            if not task.is_completed and task.due_date:
                ...
                if timedelta(0) < time_until_due <= timedelta(hours=1) and \
                   (task.due_date - timedelta(minutes=self._check_interval_seconds / 60)).date() != now.date(): # Check if it's a new reminder since last check
                    message = f"Lembrete: A tarefa '{task.description}' tem prazo às {task.due_date.strftime('%H:%M')}!"
                    ...
                    message = f"ATENÇÃO: A tarefa '{task.description}' está atrasada!"
                    await self.notification_manager.add_notification(message, level=NOTIFICATION_ERROR, vocalize=True)
                    await self.tts_module.speak(message)
                    self.logger.warning(f"Tarefa '{task.description}' está atrasada.")
    ...
    async def _add_task_command(self, description: str, due_date: Optional[str] = None, priority: int = 0) -> Dict[str, Any]:
        ...
        `priority` pode ser 0 (Baixa), 1 (Média) ou 2 (Alta).
        """
        ...
        if due_date:
            try:
                ...
                await self._speak_response("Formato de data/hora inválido para o prazo. Por favor, use 'AAAA-MM-DD HH:MM'.")
                return {"success": False, "output": "Formato de data/hora inválido.", "error": "Invalid date/time format"}

        if not (0 <= priority <= 2):
            await self._speak_response("A prioridade deve ser 0 (Baixa), 1 (Média) ou 2 (Alta).")
            return {"success": False, "output": "Prioridade inválida.", "error": "Invalid priority value"}
    ...
    async def _list_tasks_command(self, status: str = "pending", limit: int = 5) -> Dict[str, Any]:
        ...
        `status` pode ser "pending" (pendente), "completed" (concluída) ou "all" (todas).
        """
        await self._tasks_loaded.wait()
        ...
        if not tasks_to_display:
            message = f"Não há tarefas '{status}' para listar."
            await self._speak_response(message)
            return {"success": True, "output": message, "error": None}
        
        output_lines = [f"Tarefas ({status}):"]
        priority_map = {0: "Baixa", 1: "Média", 2: "Alta"}
        for i, task in enumerate(tasks_to_display):
            due_date_str = task.due_date.strftime('%Y-%m-%d %H:%M') if task.due_date else 'N/A'
            completion_status = "✅ Concluída" if task.is_completed else "⏳ Pendente"
        ...
        message = "\n".join(output_lines)
        await self._speak_response(f"As suas tarefas '{status}' foram listadas. Verifique o ecrã para os detalhes.")
    ...
    async def _mark_task_completed_command(self, task_id_prefix: str, completed: bool = True) -> Dict[str, Any]:
        ...
        if task_to_update:
            ...
            status_text = "concluída" if completed else "marcada como pendente novamente"
        ...
        if task_to_update:
            ...
            if new_due_date:
                try:
                    ...
                    await self._speak_response("Formato de data/hora inválido para o novo prazo. Por favor, use 'AAAA-MM-DD HH:MM'.")
                    return {"success": False, "output": "Formato de data/hora inválido.", "error": "Invalid date/time format"}
            
            if new_priority is not None:
                if not (0 <= new_priority <= 2):
                    await self._speak_response("A nova prioridade deve ser 0 (Baixa), 1 (Média) ou 2 (Alta).")
                    return {"success": False, "output": "Prioridade inválida.", "error": "Invalid priority value"}
    ...
    async def _delete_task_command(self, task_id_prefix: str) -> Dict[str, Any]:
        ...
        if len(matching_tasks) == 1:
            ...
            message = f"Nenhuma tarefa encontrada com o ID '{task_id_prefix}' para remoção."
            await self._speak_response(message)
            return {"success": False, "output": message, "error": "Task not found"}
        ...
        return [
            {
                ...
                "function": {
                    "name": "add_task",
                    "description": "Adiciona uma nova tarefa à lista de afazeres do usuário. Pode incluir um prazo e uma prioridade.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "description": {
                            ...
                            "priority": {
                                "type": "integer",
                                "description": "A prioridade da tarefa: 0 para Baixa, 1 para Média, 2 para Alta. Padrão para 0.",
                                "enum": [0, 1, 2],
                                "default": 0
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    "name": "list_tasks",
                    "description": "Lista as tarefas da lista de afazeres do usuário. Pode filtrar por status ('pending', 'completed', 'all') e limitar o número de resultados.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "status": {
                                "type": "string",
                                "description": "O status das tarefas a listar: 'pending' (pendente), 'completed' (concluída) ou 'all' (todas). Padrão para 'pending'.",
                                "enum": ["pending", "completed", "all"],
                                "default": "pending"
                            },
                            "limit": {
                                "type": "integer",
                                "description": "O número máximo de tarefas a retornar. Padrão para 5.",
                                "default": 5,
                                "minimum": 1
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Marca uma tarefa existente como concluída ou a retorna para o status pendente. Requer o ID completo ou um prefixo único da tarefa.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "task_id_prefix": {
                                "type": "string",
                                "description": "O ID completo ou um prefixo único da tarefa a ser marcada.",
                            },
                            "completed": {
                                "type": "boolean",
                                "description": "Defina para 'true' para marcar como concluída, 'false' para marcar como pendente. Padrão para 'true'.",
                                "default": True
                            }
                        },
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Atualiza os detalhes de uma tarefa existente. Requer o ID completo ou um prefixo único da tarefa. Pode atualizar a descrição, o prazo e a prioridade.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "task_id_prefix": {
                                "type": "string",
                                "description": "O ID completo ou um prefixo único da tarefa a ser atualizada.",
                            },
                            "new_description": {
                                "type": "string",
                                "description": "Uma nova descrição para a tarefa. Opcional.",
                            },
                            "new_due_date": {
                                "type": "string",
                            ...
                            "new_priority": {
                                "type": "integer",
                                "description": "Uma nova prioridade para a tarefa: 0 (Baixa), 1 (Média), 2 (Alta). Opcional.",
                                "enum": [0, 1, 2]
                            }
                        },
                        "required": ["task_id_prefix"],
            ...
            },
            {
                ...
                "function": {
                    "name": "delete_task",
                    "description": "Remove uma tarefa da lista de afazeres. Requer o ID completo ou um prefixo único da tarefa. Esta ação é irreversível.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "task_id_prefix": {
    ...
    def shutdown(self) -> None:
        ...
        self.logger.info("TodoManager a ser desligado. A parar a tarefa de verificação de tarefas.")
        if self._check_task:
...
if __name__ == "__main__":
    ...
    async def run_todo_manager_tests():
        ...
        assert "Tarefa 'Comprar leite e pão' adicionada com sucesso." in result_add_task1["output"]
        ...
        # --- Teste 2: Listar Tarefas Pendentes (padrão) ---
        ...
        assert "Comprar leite e pão" in result_list_pending["output"]
        assert "Enviar relatório mensal" in result_list_pending["output"]
        assert "Pagar conta de luz" in result_list_pending["output"]
        assert "Prioridade: Média" in result_list_pending["output"]
        assert "Prioridade: Alta" in result_list_pending["output"]


        # --- Teste 3: Marcar Tarefa como Concluída ---
        print("\n--- Teste 3: Marcar Tarefa como Concluída ---")
        task_id_to_complete = next(t.id for t in todo_manager._tasks.values() if t.description == "Comprar leite e pão")
        ...
        assert "Tarefa 'Comprar leite e pão' (ID:" in result_mark_complete["output"] and "concluída." in result_mark_complete["output"]

        # Verificar se não aparece mais nas pendentes
        result_list_pending_after = await dummy_gem.command_executor.execute("list_tasks")
        print(result_list_pending_after["output"])
        assert "Comprar leite e pão" not in result_list_pending_after["output"]

        # --- Teste 4: Listar Tarefas Concluídas ---
        print("\n--- Teste 4: Listar Tarefas Concluídas ---")
        ...
        assert "Comprar leite e pão" in result_list_completed["output"]
        assert "✅ Concluída" in result_list_completed["output"]

        # --- Teste 5: Atualizar Detalhes da Tarefa ---
        print("\n--- Teste 5: Atualizar Detalhes da Tarefa ---")
        task_id_to_update = next(t.id for t in todo_manager._tasks.values() if t.description == "Enviar relatório mensal")
        ...
        result_update = await dummy_gem.command_executor.execute(
            ...
            new_description="Preparar e enviar relatório trimestral",
            new_due_date=new_due_date_update,
            new_priority=0 # Baixa
        )
        print(result_update["output"])
        assert result_update["success"] is True
        assert "Tarefa 'Preparar e enviar relatório trimestral' (ID:" in result_update["output"] and "atualizada com sucesso." in result_update["output"]
        
        # Verify update
        updated_task = todo_manager._tasks[task_id_to_update]
        assert updated_task.description == "Preparar e enviar relatório trimestral"
        ...
        print("\n--- Testes do TodoManager concluídos com sucesso. ---")
